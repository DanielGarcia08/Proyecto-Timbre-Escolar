...
#define USING_HOST_NAME     true
  ...
  char server[] = "172.20.20.243"; // IP Raspberry
#else
  IPAddress server(172, 20, 20, 243); // IP Raspberry
#endif

uint16_t server_port = 3306;    // PORT MARIADB

// SELECT
const char query[] = "SELECT ID, Dies, date_format(Hora, '%H:%i') AS Hora FROM bdd_timbre.mecanic WHERE Hora = DATE_FORMAT(NOW(),'%H:%i') AND Dies = DAYNAME(NOW())";
...
void loop()
{
  ...
  delay(1000); // El codi s'executa cada minut, si no hi ha dades programades per una hora, doncs no sonara, no sortira la select
               // Si per exemple un minut no es suficient es redueix a 10s ---> delay(100) o 20s --> delay(200) 
}
